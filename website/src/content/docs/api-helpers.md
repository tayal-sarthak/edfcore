---
title: "API: helpers"
description: Signal lookup, envelopes for plotting, chunk assembly, BioSemi triggers and the text formatters — the helpers around the core read path.
section: Reference
order: 7
...
it — `header`, `validate`, `events`, `signals`, `gaps` and `json` are these helpers with an exit
code attached.
