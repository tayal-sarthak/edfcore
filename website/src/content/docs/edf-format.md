...
Onsets are seconds relative to the header's startdate and starttime, and they are decimal text. Parsed digit by digit they are exact to the resolution you parse them into, and `parseFloat` is the first thing that makes them inexact — it is not the only one.

The grammar above puts no bound on `1*DIGIT` after the point, and a writer emitting full double precision produces onsets like `+1234.5678901234`. edfcore parses those into 100 ns ticks, so the eighth fractional digit and everything after it is dropped: `+1.00000009` is exactly one second, `+0.12345678` is `1234567` ticks, and the parse reports success either way. That is a deliberate resolution, not a rounding accident — 100 ns is the unit `onsetTicks` and every window in the library are counted in — but it is a second place a decimal onset stops being exact, and the one a reader of this page would not expect. `EdfAnnotation.onsetRaw` and `durationRaw` keep the digits as written, which is what they are for.
