...
section: Guides
order: 4
lead: EDF+ stores events as text inside a signal, not in a table beside the samples. This page covers what that means for reading them, the two onset conventions every reader has to choose between, and the integer arithmetic that turns an event time into a sample index without drifting.
---
...
either family, because the label names the channel's *role*), and in every data record that
signal's block of bytes holds text instead of samples.
...
The region is exactly `samplesPerRecord * bytesPerSample` bytes — the annotations signal declares
a `samplesPerRecord` like any other, and that number is what buys the writer room for text. A
signal declaring 60 samples in a 2-byte EDF file gives every record 120 bytes of annotation space:
...
discarded rather than continued into the next signal's block, because the bytes past the bound are
samples, and reading them as text is how a reader invents annotations that are not in the file.
...
The record range is required and has no default. That is not pedantry about arguments — it is the
cost of the operation made visible in your own source. Annotation regions are interleaved with the
samples inside each record, so there is no way to read only the events. A full-file scan reads
...
`{ start: 0, count: recording.header.recordCount }` at the call site means nobody does that by
accident, and nobody has to read the library's source to find out whether they did. See
[large files](/docs/large-files) for the read patterns of every other call.
...
dropped: an empty text is how the grammar terminates a timestamp, and every record would otherwise
carry a phantom event.
...
## The timekeeping TAL, and why it is not in the list
...
does not get its second TAL promoted into the timekeeping role — that would turn a real event into
a timestamp and delete it from the list.

edfcore strips it from `annotations` because it is structure, not an event, and surfaces it
separately:
...
There is always exactly one entry per record, with no holes and no sentinel. A record whose
timekeeping TAL is missing or unparseable gets the derived onset
`start + recordIndex * recordDuration`, and a `TIMEKEEPING_TAL_MISSING` diagnostic naming that
record, so the derivation is never invisible. This array is the primitive every timeline in edfcore
is built from — see [discontinuous recordings](/docs/discontinuous).

A plain EDF or BDF has no annotations signal at all, and `readAnnotations` still works on such a
file: it returns an empty list with no diagnostics, and `recordOnsetTicks` is the arithmetic grid
`recordIndex * recordDuration`, because that is what a record onset means in a file that does not
store one.
...
The spec's answer (EDF+ 2.2.4) is the header's startdate and starttime. But the header start time
is only accurate to the second — it is an `hh.mm.ss` field — and the true start of record 0 is the
onset in record 0's own timekeeping TAL, which is allowed to be a sub-second value in `[0, 1)`. So
the two answers differ by that offset, and EDFlib, pyEDFlib and MNE all report the second one.
...
They differ by `recording.timeline.startOffsetSeconds`, which is record 0's timekeeping onset — in
the example file, `0.25`:
...
`onsetSecondsFromFirstRecord` is the one that lines up with everything else edfcore reports.
`t = 0` in this library is the start of record 0, and `chunk.startSeconds`, `segment.startSeconds`,
`gap.durationSeconds` and the bounds you pass to `readWindow` and `trimToWindow` all live on that
axis. Reach for `onsetSecondsFromHeaderStart` when you are reconciling against a wall clock, a
paper report, or another tool that quotes the on-disk value.

This is two fields rather than one field plus an option on purpose. An option would put the
convention at the *call* site while the mistake happens at the *reading* site, hundreds of lines
away, where a number called `onset` gives no indication of which one it is. And the failure it
produces is a sub-second shift of every event in the recording: too small to look wrong, large
enough to move an ERP epoch onto the other side of a stimulus.

> **Note**
> When the decoded range does not start at record 0, record 0's start is not in the bytes you
> asked for, so edfcore derives it as `onset(first decoded record) − recordIndex × recordDuration`.
> That is exact for a contiguous file and wrong by the elapsed gaps for an EDF+D one, so the
> derived value is used only when it lands inside `[0, 1)`. Otherwise rebasing is switched off and
> the two fields come back equal. Decode from record 0 whenever you intend to use
...
path. An onset written `+0.1` and one written `+0.3` are integers by the time you see them, so
equality, ordering and subtraction are decided by the digits on disk rather than by binary
floating-point rounding. Sorting, grouping into epochs, and testing whether two markers coincide
should all use `onsetTicks`:
...
that is the difference between a pre- and a post-stimulus sample. The extra digits still have to be
digits: `+1.00000000x` fails the grammar and is reported rather than silently accepted.
...
the same thing on disk, and edfcore does not invent a distinction the bytes do not make. A duration
is never signed; a `0x15` followed by something that is not `1*DIGIT [ "." 1*DIGIT ]` makes the
whole TAL malformed, and it is skipped with a `TAL_MALFORMED` diagnostic rather than half-read.

## Negative onsets are normal
...
edfcore reports `NEGATIVE_ANNOTATION_ONSET` for it, at severity `info` — meaning the file is
correct and the note exists because the situation surprises people, not because anything needs
fixing. It is emitted once per `readAnnotations` call, not once per event.

The consequence for your code is that any arithmetic on onsets has to handle negatives properly.
`bigint` division truncates toward zero, which is not the same as flooring, and getting that wrong
puts a pre-stimulus event one sample later than it belongs. The worked example below floors
...
A trailing `@@` with nothing after it is not a channel label — the text keeps it verbatim. `@@Fp1`
with nothing before it gives an empty description and the channel, because the run itself is not
empty and so is still a real annotation.

`channelLabel` is the text the file wrote. It is not resolved to a signal, and edfcore never
assumes it matches one — the writer may have scoped an event to a channel that is not in this file.
Resolve it yourself when you need the signal, and be ready for it to fail:
...
`findSignals` returns every match, which is the right shape here: real files ship duplicate labels
(CHB-MIT has `T8-P8` twice), and `getSignal` throws `EdfAmbiguousChannelError` for them rather than
picking one.
...
The first TAL of signal 2 is an ordinary annotation. Stripping it — treating "first TAL" as a
property of every annotations signal rather than of the file's first one — would silently delete a
real event from every record.
...
Note what that costs you: leaving out the file's first annotations signal means no timekeeping TAL
is read at all, so every entry of `result.recordOnsetTicks` falls back to the nominal grid. That is
silence you asked for, not a defect in the file, and no diagnostic is emitted for it. If you need
both the events of a secondary signal and true record onsets, read them all.

Passing a data signal index is refused with a plain `RangeError` — not an `EdfError`, because
parsing samples as text can only ever be a caller's mistake and never a file's:
...
detected rather than silently replaced with `U+FFFD`. When a run fails, it falls back to
ISO-8859-1 and says so:
...
Latin-1 is a guess, and it is the useful one because it round-trips every byte to exactly one
character. If your writer used a different code page, `byteOffsetInRecord` and the diagnostic's
`rawBytes` give you the bytes to re-decode yourself.
...
This is the conversion readers get wrong. The temptation is `Math.round(onset * sampleRateHz)`,
and it drifts: `sampleRateHz` is derived, and a file declaring 256 samples in a 3-second record has
a rate of 85.333… Hz that no float holds exactly. The error grows with time, and it is `undefined`
outright when the record duration is zero, which is legal EDF.
...
moves the event onto the same axis as `chunk.startSeconds` and `trimToWindow`'s bounds. Both
values are exact ticks, so nothing is lost. (`onsetSecondsFromFirstRecord` is this same quantity as
a float — use it for display, and the ticks for arithmetic.)
...
`floor(t × samplesPerRecord / recordDuration)`. Written in ticks, that is one `bigint` multiply and
one floored divide — no sample rate, no division before the comparison, no float bound. The floor
matters for negative onsets: `bigint` division truncates toward zero, which would round a
