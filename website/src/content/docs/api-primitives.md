...
- **Detecting truncation.** A header declaring 2880 records over a file that only holds 2879 whole ones is a truncated download. edfcore reports `TRUNCATED_FILE` and sets `recordCount` to what is actually present. The difference between those two numbers is not the only evidence: a writer that flushes its record-count field as it goes and then dies mid-record leaves the count agreeing with the whole records on disk and a fraction of one more after them. That earns `PARTIAL_FINAL_RECORD`, with `recordCount` unchanged and `recordCountSource` still `'headerField'`. Both findings need the file size, and without it neither is reachable — a header-only buffer looks like a complete file whose records all happen to be missing.
...
| `maxItems` | `number` | all | Show at most this many, then a dim `... and N more`. `Infinity` means no cap; a fractional limit floors. `NaN` is refused with a `RangeError`, because an option computed from an absent config key is a mistake rather than a request for the default, and so is anything that is not a number at all — a `--limit` flag, a query parameter and a config key all arrive as text, and until 0.6.115 `'3'` was coerced and printed three blocks with nothing saying so. `0` and any negative render no blocks, leaving that notice alone — this function has no summary line of its own. |
| `redactFields` | `readonly string[]` | none | Field names whose CONTENT is replaced with `[redacted]`. Pass `['patientId', 'recordingId']` before a report leaves your machine: a diagnostic quotes the raw bytes as written, and for an identification field those bytes are a person's name and birth date. The code, severity, byte offset and rule still print. A name outside the redactable vocabulary is refused rather than ignored — and so is a bare name where the list belongs: a string is iterable, so `redactFields: 'patientId'` walked its characters and reported `"p"` until 0.6.143. |
