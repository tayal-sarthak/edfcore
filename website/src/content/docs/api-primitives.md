...
The expression is `physical = bitValue * (offset + digital)`, and it is **pinned**. It is numerically worse than the obvious `physicalMinimum + (digital - digitalMinimum) * gain`. It is EDFlib's exact form, kept verbatim so that edfcore's float64 output *can* be compared bit for bit against pyEDFlib and EDFlib. That parity is the design target. It isn't yet demonstrated by a golden-value harness, so treat it as intent rather than a measured guarantee. The two forms disagree by up to about 9.3e-10 LSB (ten orders of magnitude below the quantisation floor). On asymmetric ranges they differ on nearly half the samples by one ULP. If your pipeline cross-validates against a Python reader, that shared expression is why the numbers are expected to agree.
