...
The `node:` import lives in exactly one file, and a packaging test greps the built universal bundle for that scheme prefix.
...
> **Note**
> edfcore is at 0.1.0. Alongside 1,100+ tests on generated fixtures, it's checked against public corpora it did not author: the teuniz.net EDF/EDF+/BDF+ test files and PhysioNet's sleep-edfx. That includes a real 22-hour polysomnography recording. Those checks are numeric rather than smoke tests. What is still missing is an element-by-element comparison against pyEDFlib and MNE, so no claim of numeric parity with those readers is made here. Treat the API surface as still able to move.
