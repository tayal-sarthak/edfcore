...
The date checks are the ones most likely to matter. `DATE_UNPARSEABLE` covers two different refusals and `diagnostic.field` says which: `startDate` means the file has no calendar date at all, and `startTime` means it has no wall clock. In the second case `startTime.clock` is a substituted midnight — `clockSource` is `'none'`, `formatHeader` prints `unknown`, and `formatStartTimeNaive` returns `undefined`. Elapsed times are unaffected, but there's no day to put them on. `DATE_FIELDS_DISAGREE` means the `dd.mm.yy` header field and the EDF+ `Startdate` subfield name different days. Both are exposed on `header.startTime`, and edfcore reports which one it used. `DATE_IMPLAUSIBLE` covers a day that does not exist and a patient birthdate after the recording start. That's usually a two-digit year resolved through the 1985-2084 rule for a recording made outside that window.
